    ...
    <div className="bg-white dark:bg-slate-900">
      ...
      <section className="bg-gradient-to-b from-violet-50/60 dark:from-slate-800 to-white dark:to-slate-900 py-14 px-4 text-center border-b border-slate-100 dark:border-slate-700/50">
        <div className="max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-white dark:bg-slate-800 text-violet-600 dark:text-violet-400 text-xs font-bold px-4 py-1.5 rounded-full mb-5 border border-violet-100 dark:border-violet-800/40 shadow-sm">
            🖼️ SaveFromInternet · TikTok Photo Downloader
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={n} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm text-center">
                <div className="w-12 h-12 bg-gradient-to-r from-violet-500 to-pink-500 rounded-full flex items-center justify-center text-white font-extrabold text-xl mx-auto mb-4">{n}</div>
      ...
      <section className="bg-white dark:bg-slate-900 py-14 px-4 border-b border-slate-100 dark:border-slate-700/50">
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: '🖼️', title: 'Original Quality', desc: 'Full-resolution images, no compression added.' },
            { icon: '🗜️', title: 'ZIP Download', desc: 'Download all carousel images in one ZIP file.' },
            { icon: '🆓', title: 'Completely Free', desc: 'No account, no app, no hidden limits.' },
            { icon: '📱', title: 'All Devices', desc: 'iPhone, Android, PC, Mac — any browser.' },
          ].map(({ icon, title, desc }) => (
            <div key={title} className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700/50 text-center">
      ...
      <section className="bg-slate-50 dark:bg-slate-800/50 py-14 px-4 border-b border-slate-100 dark:border-slate-700/50">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <p className="flex gap-2"><span className="text-emerald-500 font-bold">✓</span><span><strong className="text-slate-900 dark:text-white">Single photo posts</strong> — any standalone image shared on TikTok</span></p>
            <p className="flex gap-2"><span className="text-emerald-500 font-bold">✓</span><span><strong className="text-slate-900 dark:text-white">Photo carousels</strong> — all images in a multi-photo slideshow post</span></p>
            <p className="flex gap-2"><span className="text-emerald-500 font-bold">✓</span><span><strong className="text-slate-900 dark:text-white">Image galleries</strong> — collections shared as TikTok Stories or posts</span></p>
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              ...
              <Link key={href} href={href} className="px-4 py-2 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-700 hover:border-rose-200 rounded-full text-sm font-medium transition-all">
                {label}
              </Link>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="bg-gradient-to-r from-violet-500 to-pink-500 py-14 px-4 text-center">
        <h2 className="text-3xl font-extrabold text-white mb-4">Download TikTok Photos — Free</h2>
        <p className="text-violet-100 mb-8 text-lg">Original quality · Carousel support · SaveFromInternet.com</p>
        <Link
          href="/"
          className="inline-block bg-white text-violet-600 font-bold px-8 py-4 rounded-xl text-lg shadow-xl hover:scale-105 transition-all"
        >
          ⬇ Download TikTok Photo
        </Link>
      </section>
